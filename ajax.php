<?php
	session_start();
	require_once("script/_php/lib.php");
	if(isset($_REQUEST['ari']) || $_REQUEST['ari'] != NULL || $_REQUEST['ari'] != ""){
		switch($_REQUEST['ari']){
			case 1: //Image Preloader
				$root = __DIR__;
				$dir = "/img";
				$img = array();
				if(isset($_REQUEST['a']) && $_REQUEST['a'] === 1){ 
					$files = glob($root.$dir."/*.{jpeg,jpg,gif,png,svg}", GLOB_BRACE);
					$s = "";
					for($i = 0;$i < count($files); $i++){
						$img[] = str_replace($root,"",$files[$i]);
					}
				}else{
					$img = array_merge($img,parseImgs($root."/index.php"));
					$img = array_merge($img,parseImgs($root.$_SESSION['Page']['path-file']));
				}
				$data = array();
				$data[0] = $_SESSION['Page'];
				$data[1] = $img;
				header("Content-Type: application/json");
				echo json_encode($data);
			break;
			case 2:
				$page = $_REQUEST['p'];
				$exist_i = false;
				$exist_f = false;
				$root = __DIR__;
				
				for($i = 0; $i < count($_SESSION['Pages']); $i++){ if($_SESSION['Pages'][$i]['path-ui'] == strtolower($page)){ $exist_i = true; $_SESSION['Page'] = $_SESSION['Pages'][$i]; break;} }
				$exists_f = file_exists(ltrim($_SESSION['Page']['path-file'],"/"));
				if(!$exist_i || !$exists_f){ $_SESSION['Page'] = $_SESSION['Pages'][0];	}
				ob_start();
				include ltrim($_SESSION['Page']['path-file'],"/");
				$html = ob_get_clean();
				
				$hdoc = new DOMDocument();
				$hdoc->loadHTMLFile($root.$_SESSION['Page']['path-file']);
				$hdoc->preserveWhiteSpace = false;
				$images = $hdoc->getElementsByTagName('img');
				if($images->length >= 1){ foreach($images as $i){ $img[] = $i->getAttribute('src'); } }
				
				$data = array($html,$img,$_SESSION['Page']);
				header("Content-Type: application/json");
				echo json_encode($data);
			break;
			default:
				echo "BAD REQUEST";
			break;
		}
	}else{ echo "BAD REQUEST"; }
?>