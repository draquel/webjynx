<?php
	...
	if(!isset($_SESSION['Users']) || $_SESSION['Reset']){
		...
		catch(PDOException $e){	error_log("SQL Header->Users:".$sql); error_log("SQL ERROR: ".$e->getMessage()); error_log("SQL Stack Trace: ".debug_print_backtrace()); return false; }
		...
		if(isset($_REQUEST['bpg'])){
			switch($_REQUEST['bpg']){
				...
				case "p":
					...
					$page = $_SESSION['Blog']->getContentPage($pdo,$_REQUEST['bpgi']);
	...
	if($_SESSION['Page']['path-file'] == "/page/media.php"){
		if(isset($_REQUEST['mpg'])){
			switch($_REQUEST['mpg']){
				...
				case "d":
					$_SESSION['Page']['Current'] = NULL; $found = false;
					$page = $_SESSION['Media']->getContentPage($pdo,$_REQUEST['mpgi']);
