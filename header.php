<?php
	...
	if(!isset($_SESSION['Media']) || $_SESSION['Reset']){
		...
		$_SESSION['Media']->load($_SESSION['db']->con($_SESSION['dbName']),false,true);
	...
	if($_SESSION['Page']['path-file'] == "/page/media.php"){
		if(isset($_REQUEST['mpg'])){
			switch($_REQUEST['mpg']){
				...
				case "d":
					$_SESSION['db']->connect($_SESSION['dbName']); $_SESSION['Page']['Current'] = NULL; $found = false;					
					$page = $_SESSION['Media']->getContentPage($_SESSION['db']->con($_SESSION['dbName']),$_REQUEST['mpgi']);
				...
				case "admin": $_SESSION['Page']['meta-title'] = "Media Administration"; $_SESSION['Page']['meta-description'] = "Blog Admin Console"; if($_REQUEST['mpgi']){ $_SESSION['Page']['meta-title'] .= " - ".ucfirst($_REQUEST['mpgi']); } break;
			}
		}else{
			$_REQUEST['mpg'] = NULL;
			if(isset($_REQUEST['mpgi']) && $_REQUEST['mpgi'] != NULL && $_REQUEST['mpgi'] != ""){ $_SESSION['Page'] = $_SESSION['Pages'][0]; $_SESSION['Error']['404']['path-ui'] = $_SERVER['REQUEST_URI']; }
